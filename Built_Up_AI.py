...
class MainThread(QThread):
    ...
    def run_jarvis(self):
        self.wish()
        self.talk(
            'Hello boss I am jarvis your assistant. please tell me how can i help you')
        while True:
            # Every time taking command after a task is done
            ...
            # Eg: jarvis check covid (or) corona status
                ...
                self.talk(
                    "Boss which state covid 19 status do you want to check")
                s = self.take_Command()
                self.Covid(s)
            ...
            # Eg: Jarvis start Screen recording
                ...
                try:
                    ...
                    self.talk(
                        "Boss an unexpected error occured couldn't start screen recording")
            # Command for phone number tracker
            elif ("track" in self.command) or ("track a mobile number" in self.command):
                self.talk(
                    "Boss please enter the mobile number with country code")
                try:
                    ...
                    self.talk(
                        "Boss location of the mobile number is saved in Maps")
                except:
                    self.talk(
                        "Boss an unexpected error occured couldn't track the mobile number")
            ...
            # Eg: Jarvis play music
                ...
                try:
                    # change the song path directory if you have songs in other directory
                    music_dir = 'E:\\music'
            ...
            # Eg: jarvis you can sleep now
                ...
                self.talk(
                    "Okay boss, I am going to sleep you can call me anytime.")
                break
            # command for waking the jarvis from sleep
            # jarvis wake up
            elif ("wake up" in self.command) or ("get up" in self.command):
                self.talk(
                    "boss, I am not sleeping, I am in online, what can I do for u")
            ...
            elif ('Gesture' in self.command) or ('Hand Control' in self.command): 
                self.Gesture(self.command)
    ...
    def Mobilecamra(self):
        ...
        try:
            ...
            # Discription for this is available in the README file
    ...
    def Covid(self, s):
        try:
            ...
            Death = ss["Death"]
            print(
                f"Boss the total cases in {s} are {Total}, the number of active cases are {Active}, and {Cured} people cured, and {Death} people are death")
            self.talk(
                f"Boss the total cases in {s} are {Total}, the number of active cases are {Active}, and {Cured} people cured, and {Death} people are death")
            time.sleep(5)
            ...
            if F:
                ...
                if "group" in command:
                    kit.sendwhatmsg_to_group(
                        numberID, message, int(hour), int(min)+1)
                else:
                    kit.sendwhatmsg(numberID, message, int(hour), int(min)+1)
                self.talk("Boss message have been sent")
            if F == False:
                self.talk(
                    f'Boss, the name not found in our data base, shall I add the contact')
    ...
    def Display(self):
        ...
        ContactsFile = open("Contacts.txt", "r")
        self.talk(
            f"Boss displaying the {count} contacts stored in our data base")
        for line in ContactsFile:
            s = line.split("\"")
            print("Name: "+s[1])
            print("Number: "+s[3])
    ...
    def NameIntheContDataBase(self, command):
        ...
        if bo:
            print(
                f"Contact Match Found in our data base with {name} and the mboile number is {number}")
            self.talk(
                f"Boss Contact Match Found in our data base with {name} and the mboile number is {number}")
        else:
            self.talk(
                "Boss the name not found in our data base, shall I add the contact")
            AddOrNot = self.take_Command()
            print(AddOrNot)
    ...
    def Fun(self, command):
        ...
        if 'your name' in command:
            ...
            self.talk(
                "you are studing in Amrita Vishwa Vidyapeetam, with batcheloe in Computer Science and Artificail Intelligence")
    ...
    def brows(self, command):
        ...
        if 'google' in command:
            ...
            # path for your edge browser application
    ...
    def yt(self, command):
        ...
        if 'play' in command:
            ...
            self.talk(
                "Boss please enter the youtube video link which you want to download")
            link = input("Enter the YOUTUBE video link: ")
            yt = YouTube(link)
            yt.streams.get_highest_resolution().download()
            self.talk(
                f"Boss downloaded {yt.title} from the link you given into the main folder")
    ...
    def scshot(self):
        ...
        self.talk(
            "Please boss hold the screen for few seconds, I am taking screenshot")
        time.sleep(3)
